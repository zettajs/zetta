  ...
  this.server.on('upgrade', function(request, socket, headers) {
    ...
    if (/^\/peers\/(.+)$/.exec(request.url)) {
      async.eachSeries(self._wsHooks.peerConnect, function(handler, next) {
        ...
        // Transport protocol is set by the client or falls back to spdy/3.1 for
        // existing zetta clients.
        let protocol = request.headers[Constants.TransportProtocolHeader] 
                        || Constants.FallbackTransportProtocol;

        // If protocol is unknown disconnect peer.
        if (!Constants.TransportProtocols.includes(protocol)) {
          return sendError(400);
        }

        // Handle Peer Request
        self.wss.handleUpgrade(request, socket, headers, function(ws) {
          self.setupPeerSocket(ws, request, { protocol: protocol });
        });
      });
...
ZettaHttpServer.prototype.setupPeerSocket = function(ws, request, options) {
  var self = this;
  ...
  // Merge options with HTTP peer options.
  Object.entries(self.peerOptions).forEach(entry => { options[entry[0]] = entry[1];})
  ...
  if (self.peers[name] && self.peers[name].state !== PeerSocket.DISCONNECTED) {
    ...
    self.peers[name].init(ws, request, options);
  } else {
    // TODO(adammagaluk): Cleamup multiple ways of initializing the PeerSocker.
    // init() should be required after the constructor.
    var peer = new PeerSocket(ws, request, name, self.peerRegistry, options);
    ...
    peer.on('error', function(err) {
      // TODO(adammagaluk): If no subscriptions for the peer are active we can
      // safely remove the peer from http servers list.
