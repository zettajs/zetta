  ...
  this.wss.on('connection', function(ws) {
    ...
    if (match) {
      ...
      peer.on('error', function(err) {
        ...
        self.zetta.pubsub.publish('_peer/disconnect', { peer: peer, peerId: peerId, id: peer.serverId });
      });

      peer.on('connected', function() {
        // setup router
        self.agents[peer.serverId] = peer.agent;
        self.agents[peerId] = peer.agent;
        self.router[peer.serverId] = peerId;
        self.peers.push(peer);
        self.eventBroker.peer(peer);

        self.zetta.log.emit('log', 'http_server', 'Peer connection established ' + peerId + ' mapped to ' + peer.serverId);
        self.zetta.pubsub.publish('_peer/connect', { peer: peer, peerId: peerId, id: peer.serverId });
      });

      peer.on('end', function() {
        self.zetta.log.emit('log', 'http_server', 'Peer connection closed for ' + peerId + ' mapped to ' + peer.serverId);
        self._cleanupPeer(peer); // remove from zetta internals
        self.zetta.pubsub.publish('_peer/disconnect', { peer: peer, peerId: peerId, id: peer.serverId });
      });
