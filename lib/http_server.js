...
ZettaHttpServer.prototype.setupEventSocket = function(ws) {
  ...
  // setup helpers for 
  ws._env = { helpers: {}};
  var serverId = ws.upgradeReq.headers['zetta-forwarded-server'] || this.zetta.id;
  ...
  // handle local ws queries
  if(query.name === this.zetta.id) {
    var topic = query.topic;
    var queryPrefix = 'query/';
    if (topic.slice(0, queryPrefix.length) === queryPrefix) {
      var ql = topic.slice(queryPrefix.length);
      var q = self.zetta.runtime.query().ql(ql);
      self.zetta.runtime.observe(q, function(device) {
        var loader = { path: '/servers/' + encodeURI(self.zetta.id) };
        console.log('new device' + topic)
        //          var formatted = deviceFormatter({ loader: loader, env: env, model: device });
        //          self.zetta.pubsub.publish(encodeURIComponent('query/' + ql), formatted);
      });
    }
  }
