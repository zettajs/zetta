  ...
  this.wss.on('connection', function(ws) {
    ...
    if (match) {
      ...
      peerRequest.on('error', function(err) {
        console.error('Peer request failed', err);
        ws.close(); // close socket if request failed
      });

      peerRequest.on('response', function(res) {
        console.error('Peer Request: on response');
      });

      peerRequest.on('socket', function(socket) {
        console.error('Peer Request: on socket');
      });

      peerRequest.on('connect', function(socket) {
        console.error('Peer Request: on connect');
      });

      peerRequest.on('continue', function(socket) {
        console.error('Peer Request: on continue');
      });
