...
ZettaHttpServer.prototype.init = function(cb) {
  ...
  this.wss = new WebSocketServer({ server: this.server, verifyClient: function(info, cb) {
    if (!(/^\/peers\/(.+)$/.exec(info.req.url))) {
      return cb(true);
    }

    var name = /^\/peers\/(.+)$/.exec(url.parse(info.req.url, true).pathname)[1];
    name = decodeURI(name);

    if (self.peers[name]) {
      self.zetta.log.emit('warn', 'http_server', 'Duplicate websocket connection for peer "' + name + '" disconnecting new connection.');

      return cb(false, 409, 'Peer already connected with name');
    } else {
