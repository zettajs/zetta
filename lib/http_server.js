...
var PeersResource = require('./api_resources/peers');
...
var ZettaCloud = module.exports = function(server) {
  ...
  this.cloud = argo()
    .use(titan)
    .format({ engines: [Siren], override: { 'application/json': Siren }, directory: path.join(__dirname, './formats') })
    .add(RootResource, server)
    .add(PeersResource, server)
...
ZettaCloud.prototype.init = function(cb) {
  ...
  this.wss.on('connection', function(ws) {
    ...
    if (match) {
      ...
      self.agents[appName] = spdy.createAgent(SpdyAgent, {
        host: appName,
        port: 80,
        socket: ws._socket,
        spdy: {
