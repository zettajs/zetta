...
RootResource.prototype.list = function(env, next) {
  var self = this;
  if(env.route.query.ql && env.route.query.server) {
    var ql = env.route.query.ql;
    var server = env.route.query.server;
    ...
    if(this.shouldProxy(server)) {
      ...
      this.server.peerRegistry.get(server, function(err, peer) {
        if(err) {
          ...
          var qs ='?' + querystring.stringify({ql: env.route.query.ql});
