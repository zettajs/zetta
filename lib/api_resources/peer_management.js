...
PeerManagementResource.prototype.deletePeer = function(env, next) {
  ...
  this.registry.get(id, function(err, result) {
    if(result.direction === 'initiator') {
      self._disconnect(result);
      env.response.statusCode = 200;
      next(env);
    } else if(result.direction === 'acceptor') {
      self._proxyDisconnect(env, next);
    } else {
      env.response.statusCode = 500;
      next(env);  
    }  
...
PeerManagmentResource.prototype._disconnect = function(peer) {
  var wsUrl = PeerClient.calculatePeerUrl(result.url, self.server._name);
...
PeerManagementResource.prototype._proxyDisconnect = function(env, next) { 
