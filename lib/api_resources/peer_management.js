...
PeerManagementResource.prototype.deletePeer = function(env, next) {
  var id = env.route.query.connectionId;
  var self = this;
...
PeerManagementResource.prototype._connect = function(peer) {
  ...
  self.server._peerClients.push(client);
  client.on('connecting', function() {
    peer.status = 'connecting';
    self.registry.save(peer);  
  });

  client.on('connected', function() {
    peer.status = 'connected';
    peer.connectionId = client.connectionId;
    self.registry.save(peer);
    self.server.pubsub.publish('_peer/connect', { peer: client});
  });
  ...
  client.on('error', function(error) {
    self.registry.get(peer.id, function(err, result) {
      ...
      self.pubsub.publish('_peer/disconnect', { peer: client });
  ...
  client.on('closed', function() {
    self.registry.get(peer.id, function(err, result) {
      ...
      self.server.pubsub.publish('_peer/disconnect', { peer: client});
  ...
  if (this.data && this.data.length) {
    this.base.entities = this.data.map(function(peer) {
      ...
      entity.actions.push({
        name: 'disconnect',
        method: 'DELETE',
        href: self.urlHelper.path('/peer-management'),
        fields:[{name:'connectionId', type:'text', value: peer.connectionId}]
      },{
        name: 'update',
        method: 'PUT',
        href: self.urlHelper.path('/peer-management'),
        fields: [{name: 'url', type: 'url'}, {name:'connectionId', type:'text', value: peer.connectionId}]
      }); 
...
PeerItemBuilder.prototype.actions = function() {
  ...
  this.base.actions.push({
    ...
    fields:[{name:'connectionId', type:'text', value: this.data.connectionId}]
  },{
    name: 'update',
    method: 'PUT',
    href: this.urlHelper.path('/peer-management'),
    fields: [{name: 'url', type: 'url'}, {name:'connectionId', type:'text', value: this.data.connectionId}]
  });  
