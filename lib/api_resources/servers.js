...
ServerResource.prototype.queryDevices = function(env, next) {
  ...
  env.request.getBody(function(err, body) {
    if(err) {
      ...
      var context = { server: self.server, devices: self.server.runtime._jsDevices, loader: self.getServer(env), env: env, classes:['search-results'], query: body.ql };
      if(body.server) {
        context.queryServer = body.server;
      }
      env.format.render('server', context);
      next(env);
    }
