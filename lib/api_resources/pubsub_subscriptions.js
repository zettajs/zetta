  ...
  env.request.getBody(function(err, body) {
    if(err) {
      ...
      if (body.topic) {
        ...
        self._listeners[serverId][body.topic] = env;

        var qt = querytopic.parse(body.topic);
        if (qt) {
          body.topic = querytopic.format(qt);
          self.zetta.pubsub.subscribe(body.topic, env);
          
          setImmediate(function() {
            self.zetta.httpServer.eventBroker.subscribeToDeviceQuery(body.topic);
          });
        } else {
          self.zetta.pubsub.subscribe(body.topic, env);
        }
