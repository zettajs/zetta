  ...
  env.request.getBody(function(err, body) {
    if(err) {
      ...
      if (body.topic) {
        ...
        if (body.topic.slice(0, queryPrefix.length) === queryPrefix) {
          var ql = body.topic.slice(queryPrefix.length);
          ...
          self.zetta.runtime.observe(q, function(device) {
            ...
            self.zetta.runtime.pubsub.publish(encodeURIComponent('query/' + ql), formatted);
          });
        }
      } else {
