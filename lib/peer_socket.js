...
var PeerSocket = module.exports = function(ws, appName, peerRegistry) {
...
PeerSocket.prototype._setupConnection = function(cb, tries) {
  ...
  function retry() {
    if (tries < 2) {
      self._setupConnection(cb, ++tries);
    } else {
      cb(new Error('Failed to initiate peer connection'));
    }
  }

  var requestOpts = { method: 'GET', path: '/', agent: this.agent };
...
PeerSocket.prototype._initWs = function(ws) {
  ...
  this.ws._socket.on('end', function() {
    console.log("this.ws._socket.on('end'");
    clearInterval(self._pingTimer);
    self.emit('end');
  });

  this.ws.on('close', function() {
    console.log("this.ws.on('close'");
    clearInterval(self._pingTimer);
...
PeerSocket.prototype._setPeerStatus = function(status, err, cb) {
  ...
  this.peerRegistry.get(this.serverId, function(err, peer) {
    if (err) {
      return cb(err);
    }
