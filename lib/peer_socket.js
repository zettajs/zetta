...
PeerSocket.prototype._cleanup = function() {
  ...
  // TODO: validate memory leaks in new spdy library.
...
PeerSocket.prototype._initWs = function(ws, request) {
  ...
  // Catch errors on the the Spdy Connection event emitter. In some cases they
  // emit only on that object and do not pass the `error` event into the agent.
  // spdyState.conneciton is set before calling the `_connect` event.
  // https://github.com/spdy-http2/node-spdy/blob/657c20d35906a058199c8b8d721c902ef6cdc4d6/lib/spdy/agent.js#L65-L71
  this.agent.once('_connect', function(err) {
    self.agent._spdyState.connection.once('error', function(err) {
      self.close();
      self.emit('error', err);
    })
  });
...
PeerSocket.prototype.onPushData = function(stream) {
  ...
  stream.on('end', function() {
    ...
    //TODO(adammagaluk): verify any memory leaks without closing
...
PeerSocket.prototype.subscribe = function(event, cb) {
  ...
  var wrappedCallback = function(err) {
    if (!callbackHasBeenCalled) {
      ...
      cb();
    } else if (err && self.state == STATES.CONNECTED) { // Ignore the error is the peer is disconnected.
