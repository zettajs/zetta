...
StreamTopic.prototype.match = function(topicString) {
  if (this._useComponents) {
    var components = [ this.serverName, this.deviceType, this.deviceId, this.streamName ];
    var checkTopic = StreamTopic.parse(topicString);
    var checkComponents = [ checkTopic.serverName, checkTopic.deviceType, checkTopic.deviceId, checkTopic.streamName ];
    
    return components.every(function(component, idx) {
      if (component === undefined) {
        return true;
      }
      if (component instanceof RegExp) {
        return component.exec(checkComponents[idx]);
      } else {
        return minimatch(checkComponents[idx], component);
      }
    });
  } else {
