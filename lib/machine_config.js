...
MachineConfig.prototype.stream = function(queueName, handler) {
  ...
  var dataStream = new ZettaDataStream(queue);
...
MachineConfig.prototype.monitor = function(queueName) {
  ...
  var stream = new ZettaDataStream(queue);
...
MachineConfig.prototype.buildActions = function(env) {
  ...
  Object.keys(self.streams).forEach(function(name) {
    var fields = [];
...
MachineConfig.prototype.buildEntity = function(loader, env, actions) {
  this.update();
  selfPath = env.helpers.url.current();
  var self = this;
  ...
  if (this._devices.length) {
    /*entity.entities = this._devices.filter(function(device) {
      var path = env.helpers.url.join(self.path);

      if (loader.exposed[url.parse(path).path]) {
        return device;
      }
    }).map(function(device) {
      var path = env.helpers.url.join(self.id);
      return device.buildEntity(env, self, null, path);
    });*/
