...
var WebSocket = module.exports = function(address, httpOptions) {
  ...
  var self = this;
  if (httpOptions) {
    // Merge headers into the mandatory ones above.
    var extraHeaders = httpOptions.headers;
    if (extraHeaders) {
      Object.entries(extraHeaders).forEach((entry) => {
        self.options.headers[entry[0]] = entry[1];
      })
...
WebSocket.prototype.start = function() {
  ...
  var subscription = req.subscribe(function(env) {
    ...
    if (env.response.head.length > 0) {
      env.request.connection.unshift(env.response.head);
    }
