...
var RemoteQuery = require('./remote_query');
...
Runtime.prototype.from = function(server) {
  return new RemoteQuery(server, Query.of('devices'));
};
  ...
  queries.forEach(function(query) {
    var queryObservable = observable.flatMap(function(device) {
      return Rx.Observable.create(function(observer) {
        if (query.__proto__ === RemoteQuery.prototype) {
          // set up a remote query
        } else if (query.__proto__ === Query.prototype) {
          self.registry.match(query, device, function(err, match) {
            if (match) {
