  ...
  queries.forEach(function(query) {
    if (query.remote === true) {
      ...
      var queryObservable = Rx.Observable.create(function(observer) {
        ...
        // init peer subscribes to peer for query, once at most.
        var listener = null;
        var initPeer = function(peer) {
          if (!self._remoteDevices[peer.name]) {
            self._remoteDevices[peer.name] = {};
          }

          var topic = 'query/' + ql;
          // set up reactive query with peer and call onNext when available.
          peer.subscribe(encodeURIComponent(topic));
          // if already subscribed to topic once, remove it
          if (listener) {
            peer.removeListener(topic, listener);
          }
