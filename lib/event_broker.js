  ...
  this.clients.forEach(function(client) {
    client.query.forEach(function(query) {
      if (topic.match(query.topic)) {
        ...
        if (client.streamEnabled) {
          if (query.caql) {
            self.caqlSession.data.data = { 0: { data: data.data} };
            console.log('session ', self.caqlSession.data.data);
            console.log('eb ',data.data);
            self.caqlSession.find(query.caql, function(err, results) {
              console.log(arguments);
              if (results.length === 1) {
                data.data = results[0][Object.keys(results[0])[0]];
                sendToClient();
              }
            })
            return;
