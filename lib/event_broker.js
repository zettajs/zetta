...
function isDeviceQuery(topic) {
  var queryPrefix = 'query/';
  return topic.slice(0, queryPrefix.length) === queryPrefix;
}
...
EventBroker.prototype._subscribe = function(query) {
  ...
  if (query.name === this.zetta.id) {
    ...
    if (this.subscriptions[topic].count === 0) {
      ...
      // subscribe to local
      if (isDeviceQuery(topic)) {
        console.log('adding device query - ', topic);
    ...
    if (!this._peerSubscriptions[query.name][topic]) {
      ...
      // subscribe to local
      if (isDeviceQuery(topic)) {
        console.log('adding peer device query - ', topic);
        this._subscribeToDeviceQuery(topic);
      }
    }
