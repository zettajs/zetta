...
EventBroker.prototype.peer = function(peer) {
  ...
  this._peerSubscriptions[peer.appName] = this._peerSubscriptions[peer.appName] || {};
  // subscribe to topics for interest
  Object.keys(this._peerSubscriptions[peer.appName]).forEach(function(topic) {
...
EventBroker.prototype._subscribe = function(query) {
  ...
  // is local
  if (query.appName === this.zetta.id) {
    ...
    if (!this._peerSubscriptions[query.appName]) {
      this._peerSubscriptions[query.appName] = { topic: 0};
    }

    if (!this._peerSubscriptions[query.appName][topic]) {
      this._peerSubscriptions[query.appName][topic] = 0;
      var peer = this.peers[query.appName];
      if (peer) {
        peer.removeAllListeners(topic);
        peer.on(topic, function(data) {
          self._publish(topic, data);
    ...
    this._peerSubscriptions[query.appName][topic]++;
  }
};
...
EventBroker.prototype._unsubscribe = function(query) {
  ...
  if (query.appName === this.zetta.id) {
    ...
    if (!this._peerSubscriptions[query.appName]) {
      this._peerSubscriptions[query.appName] = { topic: 1};
    }

    this._peerSubscriptions[query.appName][topic]--;
    if (this._peerSubscriptions[query.appName][topic] > 0) {
      return;
    }
    delete this._peerSubscriptions[query.appName][topic];
    var peer = this.peers[query.appName];
    if (peer) {
